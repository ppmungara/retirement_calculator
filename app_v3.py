...
def refund_for(gross, deduction, cfg, already_deducted=0.0):
    ...
    `already_deducted` is deduction that has been taken but pays no refund at
    filing — a group plan whose relief came through the paycheque. It applies to
    both sides, so it never pays out, but it still lowers the income the refund is
    measured on, which is where the contribution actually unwinds.
    """
    if deduction <= 0 or gross <= 0:
        return 0.0
    base = max(0.0, already_deducted)
    return max(0.0, total_tax(gross, cfg, base)
               - total_tax(gross, cfg, base + min(deduction, gross)))
...
class Cfg:
    ...
    source_relief: bool         # payroll already withholds less tax for the plan
    # mortgage
    mort_bal: float
    mort_rate: float
    mort_weekly: float
...
def run_scenario(cfg, invest_pct):
    ...
    # Contributions whose tax relief arrives through the paycheque rather than as a
    # refund. Held apart from `deduct` so they lower the income a refund is
    # measured on without paying one out.
    group_ytd = 0.0
    ...
    for i in range(cfg.max_months):
        ...
        if d.month == 1 and i > 0:
            ...
            pa_accum = prepay_used = extra_inc_sp = group_ytd = 0.0
        ...
        if cfg.group_is_rrsp:
            ...
            # Where payroll already withholds less tax for the plan, the relief is
            # spent by the time the return is filed, so it generates no refund.
            if cfg.source_relief:
                group_ytd += ee + er
            else:
                deduct["sp"] += ee + er
...
def coast_forward(row, months, cfg):
    """Grow the balances for `months` with nothing further paid in.

    Each account grows at its own rate — the non-registered one keeps its tax
    drag — and the after-tax figure discounts the RRSP at the withdrawal rate.
    """
    months = max(0, months)
    reg_rate = cfg.inv_return / 100 / 12
    nonreg_rate = cfg.inv_return / 100 * (1 - cfg.nonreg_drag / 100) / 12
    rrsp = row["rrsp"] * (1 + reg_rate) ** months
    tfsa = row["tfsa"] * (1 + reg_rate) ** months
    nonreg = row["nonreg"] * (1 + nonreg_rate) ** months
    total = rrsp + tfsa + nonreg
    return total, total - rrsp * cfg.rrsp_withdraw_rate / 100, rrsp, tfsa, nonreg
...
with st.sidebar:
    ...
    with st.expander("🤝 Spouse group plan"):
        ...
        source_relief = st.checkbox("Tax relief on the plan is taken at source",
                                    key="source_relief",
                                    help="On = payroll already withholds less tax for these "
                                         "contributions, so the benefit is in every paycheque "
                                         "and there is nothing left to refund at filing. Off = "
                                         "tax is withheld on full pay and the relief arrives as "
                                         "a refund. Check a paystub: is income tax calculated on "
                                         "gross, or on gross less the contribution?")
    ...
    with st.expander("🏖 Coast to retirement"):
        ...
        swr = st.number_input("Safe withdrawal rate (%)", min_value=0.5, max_value=15.0,
                              step=0.1, key="swr")
        inflation = st.number_input("Inflation (%)", min_value=0.0, max_value=15.0, step=0.1,
                                    key="inflation",
...
# ─── Coast to retirement ──────────────────────────────────────────────────────
# The point of a coast number: stop contributing the month the goal is met and let
# what is already invested carry the rest of the way to retirement.
st.divider()
st.markdown("### 🏖 Coasting to retirement")


if not detail["goal_reached"]:
    st.info(f"**{detail['name']}** never reaches the goal inside {horizon_years} years, so there "
    ...
    if months_coasting <= 0:
        st.warning(f"At **{detail['name']}** the goal lands at age {age_at_goal:.1f}, which is "
                   f"already past {retire_age}. There is nothing left to coast through — "
                   f"raise the retirement age, or the goal is simply late.")
    else:
        gross, net, rrsp_end, tfsa_end, nonreg_end = coast_forward(stop_row, months_coasting, cfg)
        # Today's dollars means discounting over everything from now to retirement,
        # which is the months spent reaching the goal plus the months coasting.
        real_divisor = (1 + inflation / 100) ** ((months_coasting + goal_month) / 12)
        gross_today, net_today = gross / real_divisor, net / real_divisor
        income = net * swr / 100
        income_today = net_today * swr / 100

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("🎯 Goal met", detail["goal_reached"], f"age {age_at_goal:.1f}")
        c2.metric("🏖 Years coasting", f"{months_coasting / 12:.1f}",
                  f"to age {int(retire_age)}")
        c3.metric(f"📊 Portfolio at {int(retire_age)}", money(gross),
                  f"from {money(stop_row['inv_bal'])}")
        c4.metric("💧 After tax", money(net), f"RRSP discounted {rrsp_withdraw_rate:.0f}%")

        d1, d2, d3 = st.columns(3)
        d1.metric("💵 Income at retirement", f"{money(income)}/yr", f"{swr:.1f}% withdrawal")
        d2.metric("📉 In today's dollars", f"{money(income_today)}/yr",
                  f"{inflation:.1f}% inflation")
        d3.metric("🛒 Today's spending", f"{money(expenses * 12)}/yr",
                  f"{money(income_today - expenses * 12)}/yr vs plan",
                  delta_color="normal")

        st.markdown(
            f'<div class="info-box">At <b>{detail["name"]}</b>: contributions stop '
            f'{detail["goal_reached"]} with {money(stop_row["inv_bal"])} invested, and '
            f'{months_coasting / 12:.1f} years of growth at {inv_return:.1f}% take it to '
            f'<b>{money(gross)}</b> by age {int(retire_age)} — {money(net)} after tax, or '
            f'{money(net_today)} in today\'s dollars. Split at that point: RRSP '
            f'{money(rrsp_end)} · TFSA {money(tfsa_end)} · non-registered {money(nonreg_end)}.'
            f'</div>', unsafe_allow_html=True)

        verdict = ("covers" if income_today >= expenses * 12 else "falls short of")
        st.caption(f"A {swr:.1f}% withdrawal on the after-tax balance {verdict} today's "
                   f"{money(expenses * 12)}/yr of spending, measured in today's dollars. "
                   f"Inflation is applied only to restate the result — the projection itself "
                   f"runs in nominal dollars, and salaries and savings grow only if you set them "
                   f"to. Nothing is contributed after the goal, and nothing is withdrawn before "
                   f"retirement.")
